...
{
    private OrderedDictionary mainInventory;
    private int maxCapacity = 15;

    private void Awake()
    {
        mainInventory = new OrderedDictionary();
    }
    ...
    {
        if (mainInventory.Count - 1 >= index) //Check for out of range
        {
            KeyValuePair<GameObject, int> temp = (KeyValuePair<GameObject, int>)mainInventory[index];
            return (KeyValuePair<GameObject, int>)mainInventory[index];
        }
    ...
    {
        ICollection value = mainInventory.Values;
        int j = 0;
        foreach (KeyValuePair<GameObject, int> i in value)
        {
            ...
            {
                mainInventory[j] = new KeyValuePair<GameObject, int>(item, i.Value + amount);
                Destroy(i.Key);
                return true;
            }
            j++;
        }
        if (mainInventory.Count < maxCapacity)
        {
            KeyValuePair<GameObject, int> itemAndAmount = new KeyValuePair<GameObject, int>(item, amount);
            mainInventory.Add(item.GetComponent<PickableItem>().itemName, itemAndAmount);
            return true;
        }
        return false;
    }
    ...
    */
    public bool Remove(string item, int amount)
    {
        ...
        foreach (KeyValuePair<GameObject, int> i in value)
        {
            ...
            {
                if (i.Value > amount)
                    mainInventory[j] = new KeyValuePair<GameObject, int>(i.Key, i.Value - amount);
                else
                {
                    Destroy(i.Key);
                    mainInventory.RemoveAt(j);
                }
                return true;
            }
            j++;
